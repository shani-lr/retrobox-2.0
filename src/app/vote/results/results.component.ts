...
export class ResultsComponent implements OnInit, OnDestroy {
  ...
  private calculateVotes() {
    this.team.vote.map((userVote: { group: string, votes: number, user: string }) => {
      const groupIndex = this.results.map(x => x.name).indexOf(userVote.group);
      if (groupIndex > -1) {
        this.results[groupIndex].value = (+this.results[groupIndex].value) + userVote.votes;
      } else {
        this.results.push({name: userVote.group, value: userVote.votes});
      }
    });
